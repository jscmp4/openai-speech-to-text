...
def build(mode="gpu"):
    ...
    with open(run_bat, "w") as f:
        ...
        f.write('cd /d "%~dp0"\n')
        f.write(':: Kill any old process on port 8080\n')
        f.write('for /f "tokens=5" %%a in (\'netstat -ano -p TCP ^| findstr ":8080.*LISTENING"\') do (\n')
        f.write('    echo Closing old instance (PID %%a^)...\n')
        f.write('    taskkill /F /PID %%a >nul 2>&1\n')
        f.write(')\n')
